...
struct TransactionDetailSheet: View {
    ...
    private var title: String {
        switch context {
        case .addTransaction:
            OSLocales.getText(.ADD_TRANSACTION)
        case let .editTransaction(transaction: transaction):
    ...
    private func handleOnAppear() {
        if case let .editTransaction(transaction) = context {
...
private final class ViewModel: ObservableObject {
    ...
    func loadContentOfImportedFile(_ data: Data) async -> [OSTransaction]? {
        ...
        do {
            ...
            logger
                .warning("Failed to read contents of CSV; description='\(error.localizedDescription)'; error='\(error)")
    ...
    private func handleErrorPopupTimer(_: Timer) {
