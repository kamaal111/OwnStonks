...
final class ForexCacheUtilTests: XCTestCase {
    func testCacheLatestWithInvalidSymbols() async throws {
        ...
        for (symbols) in cases {
            ...
            let result = try await client.cacheLatest(base: .EUR, symbols: symbols, apiCall: { base, symbols in
    ...
    func testCacheLatestWithNoCache() async throws {
        ...
        for (initialCacheContainerData) in cases {
            ...
            var called = false
            let result = await client.cacheLatest(base: .EUR, symbols: [.USD], apiCall: { base, symbols in
                called = true
    ...
    func testCacheLatestWithFullCacheFound() async throws {
        ...
        for (base, symbols, expectedResult) in cases {
            ...
            let result = try await client.cacheLatest(base: base, symbols: symbols, apiCall: { base, symbols in
                XCTFail("Should be unreachable")
    ...
    func testCacheLatestWithPartialCacheFound() async throws {
        ...
        var called = false
        let result = try await client.cacheLatest(base: .EUR, symbols: [.USD, .CAD], apiCall: { base, symbols in
            called = true
    ...
    func testCacheLatestWithFailure() async throws {
        ...
        let result = await client.cacheLatest(base: .EUR, symbols: [.USD, .CAD], apiCall: { base, symbols in
